...
const { GraphQLDirective, DirectiveLocation, specifiedDirectives } = require('graphql')

const { link, fileByPath } = require('./resolvers')
const { getDateResolver } = require('./date-type')
...
const toDirectives = ({ schemaComposer, extensions, locations: defaultLocations }) => (
...
const processFieldExtensions = ({
	...
	typeComposer.getFieldNames().forEach(fieldName => {
		...
		Object.keys(extensions).filter(name => 
			!internalExtensionNames.includes(name)
		).forEach(name => {

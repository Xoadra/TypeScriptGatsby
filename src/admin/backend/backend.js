...
import { API, AuthenticationPage } from 'netlify-cms-backend-github'
...
export default class GitHub {
	...
	constructor(config, options = {}) {
		...
		/* console.log(config)
		config = config.setIn(['backend', 'repo'], 'Xoadra/TypeScriptGatsby')
		console.log(config) */
	...
	fetchFiles = (files, { repoURL = this.api.repoURL } = {}) => {
		...
		files.forEach(file => {
			promises.push(
				new Promise(resolve =>
					sem.take(() =>
						this.api.readFile(file.path, file.sha, { repoURL }).then(data => {
							resolve({ file, data })
							sem.leave()
						}).catch((err = true) => {
							sem.leave()
			...
			branches.map(({ ref }) => {
				promises.push(
					new Promise(resolve => {
						...
						return sem.take(() =>
							this.api.readUnpublishedBranchFile(contentKey).then(data => {

...
const { GraphQLList, Kind, GraphQLInterfaceType, getNullableType, getNamedType } = require('graphql')
...
const paginate = (results = [], { skip = 0, limit }) => {
	...
	const pageCount = limit
		? Math.ceil(skip / limit) + Math.ceil((count - skip) / limit) : skip
		? 2 : 1
...
const resolveValue = (resolve, value) => (
	Array.isArray(value) ? value.map(v => resolveValue(resolve, v)) : resolve(value)
)
...
const getProjectedField = (info, fieldName) => {
	...
	const fieldEnum = getNullableType(
		getNullableType(info.returnType).getFields()
